...
function getCategories(post: BloggerPost): string[] {
  return post.categories.filter((c) => c !== "Blog" && c !== "RankrSeo" && c !== "Resources" && c !== "SEO Services" && c !== "Consultation")
}

function getPrimaryCategory(post: BloggerPost): string {
  const cats = getCategories(post)
  for (const c of allCategories.slice(1)) {
    if (cats.some((pc) => pc.toLowerCase() === c.toLowerCase())) return c
  }
  return cats[0] || "SEO"
}
                  ...
                  >
                    <div className="overflow-hidden rounded-xl border border-border bg-card shadow-sm transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-lg group-hover:shadow-primary/5 h-full flex flex-col">
                      <div className="flex h-48 items-end bg-gradient-to-br from-primary to-secondary p-4 sm:h-44">
                        <div className="flex flex-wrap gap-1.5">
                          {getCategories(post).slice(0, 2).map((cat) => (
                            <span key={cat} className="rounded-full bg-white/20 px-3 py-1 text-xs font-medium text-white backdrop-blur-sm">
                              {cat}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="flex flex-col flex-1 p-5">
                        ...
                        <h3 className="mt-3 text-lg font-semibold text-card-foreground group-hover:text-primary transition-colors line-clamp-2">
                          {post.title}
                        </h3>
