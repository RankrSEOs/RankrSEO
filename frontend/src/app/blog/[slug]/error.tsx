...
export default function BlogPostError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <div className="flex min-h-[70vh] flex-col items-center justify-center px-4 text-center">
      ...
      <h1 className="text-2xl font-bold text-foreground sm:text-3xl">Failed to load post</h1>
      <p className="mt-2 max-w-md text-muted-foreground">
        Could not load this blog post. Please try again or browse other articles.
      </p>
      <div className="mt-8 flex gap-4">
        ...
        <Link href="/blog" className="rounded-xl border border-border px-6 py-2.5 text-sm font-semibold text-foreground transition-colors hover:bg-muted">
          All Posts
        </Link>
      </div>
    </div>
